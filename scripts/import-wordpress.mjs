/**
 * WordPress WXR → Sanity import script
 * Usage: node scripts/import-wordpress.mjs
 *
 * Requires SANITY_API_TOKEN env var with write permissions.
 * Set it in your project's Vars settings before running.
 */
...
function htmlToPortableText(html) {
  ...
  for (const seg of segments) {
    ...
    // Bare text (not wrapped in a tag)
...
async function uploadImageFromUrl(imageUrl) {
  try {
    // Clean the URL (remove query strings like ?w=1024)
...
async function main() {
  ...
  for (const item of posts) {
    ...
    // Check for existing post with same slug to avoid duplicates
    ...
    // Find first image in content to use as thumbnail
    let thumbnail = null;
    ...
    // Generate a simple excerpt from first paragraph
